...
import { LIBRARY_GLOBAL_PAC_TN_DEPLOY, LIBRARY_GLOBAL_CAC_LOAD_VARIABLES, SANDBOX_SITES_REPO_URL } from "@site/src/constants/v0.4.1";
...
The TN_DEPLOY pipeline first writes its parameters and the component's inputs as variable files loadable by Ansible, then clones the <Link to={SANDBOX_SITES_REPO_URL}>6G-Sandbox-Sites</Link> repository and finally launches the playbook `{COMPONENT_TYPE}/code/component_playbook.yaml`.
...
- **`6G-Sandbox-Sites/{{ deployment_site }}/core.yaml"`**: Variables unique to each site loaded from repository <Link to={SANDBOX_SITES_REPO_URL}>6G-Sandbox-Sites</Link>
