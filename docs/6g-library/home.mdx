...
The TN_DEPLOY pipeline first writes its parameters and the component's inputs as variable files loadable by Ansible, then clones the <Link to={SANDBOX_SITES_REPO_URL}>6G-Sandbox-Sites repository</Link> and finally launches the playbook `{COMPONENT_TYPE}/code/component_playbook.yaml`.
