...
import { LIBRARY_REPO_URL, SITES_REPO_URL, LIBRARY_COMPONENT_DUMMY_PUBLIC, LIBRARY_COMPONENT_ELCM, LIBRARY_COMPONENT_TN_INIT_PUBLIC, LIBRARY_COMPONENT_TN_INIT_PRIVATE, LIBRARY_COMPONENT_ELCM_PUBLIC, LIBRARY_COMPONENT_ELCM_PRIVATE } from "@site/src/config/constants";
...
| **DEPLOYMENT_SITE** | The site where the component is to be deployed. It must be a directory inside the branch in the <Link to={SITES_REPO_URL}>6G-Sandbox-Sites</Link> repository. |
...
| **FILE**            | YAML file in which you must specify the mandatory fields from the input section of the <Link to={LIBRARY_COMPONENT_DUMMY_PUBLIC}>public.yaml</Link> file of the component in the 6G-Library. |

## Example

In this example we are going to show how to deploy the <Link to={LIBRARY_COMPONENT_ELCM}>ELCM</Link> component of the 6G-Library.

As mentioned above, the first thing needed for elcm is to deploy the tn_init component.

In this case, the tn_init <Link to={LIBRARY_COMPONENT_TN_INIT_PUBLIC}>input</Link> fields are not required since the required_when is set to false in all fields. This indicates that it takes the values specified by default in the file <Link to={LIBRARY_COMPONENT_TN_INIT_PRIVATE}>private.yaml</Link>. However, you can modify the values by creating a yaml file locally and attaching it to the pipeline. Let's set these values:
...
![Empty Jenkins Pipeline](./images/jenkins_pipeline_empty.png)

Once the deployment time has elapsed, the elcm component is deployed:

In this case, the elcm <Link to={LIBRARY_COMPONENT_ELCM_PUBLIC}>input</Link> fields are not required since the required_when is set to false in all fields. This indicates that it takes the values specified by default in the file <Link to={LIBRARY_COMPONENT_ELCM_PRIVATE}>private.yaml</Link>. However, you can modify the values by creating a yaml file locally and attaching it to the pipeline. Let's set these values:
...
![Filled Jenkins Pipeline](./images/jenkins_pipeline_filled.png)
