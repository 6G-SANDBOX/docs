...
import { TNLCM_BACKEND_REPO_URL, TNLCM_FRONTEND_REPO_URL, TNLCM_BACKEND_LOGO, SANDBOX_MARKETPLACE_TNLCM, TNLCM_REPO_TAG, ONE_REPO_VERSION, OPENNEBULA_MANAGEMENT_OPERATIONS } from "@site/src/constants/v0.5.0";
...
- Backend <Link to={SANDBOX_MARKETPLACE_TNLCM}>{TNLCM_REPO_TAG}</Link>
- Frontend <Link to={SANDBOX_MARKETPLACE_TNLCM}>{TNLCM_REPO_TAG}</Link>
...
The contextualization parameters (<Link to={OPENNEBULA_MANAGEMENT_OPERATIONS}>CONTEXT section</Link>) in the VM template controls the configuration of the service, see the table below:
