...
import { ANSIBLE_PLAYBOOKS, MARKETPLACE_REPO_URL, MARKETPLACE_URL, OPENNEBULA_TERRAFORM_PROVIDER, HASHICORP_TERRAFORM_BACKENDS_S3 } from "@site/src/config/constants";
...
- Importing task file `.global/cac/load_variables.yaml`: Import variables as hostvars into the playbook. Previously mentioned in the landing page [EXTRA section](./6g_library.mdx#extra)
...
## Final notes
To troubleshoot deployments, you might want to check the Jenkins pipeline to see where it went wrong.
TNs pursue idempotency, so retrying to deploy the TN should continue just where it left off.
However, when TNs are created in the TNLCM, it is registered with the latest commit of your selected branch.
This means, that if you want to retry a deployment after a change in your code, you should change the commit from the TNLCM mongodb database itself.
