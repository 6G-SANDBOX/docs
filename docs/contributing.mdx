...
import { SANDBOX_DOCS_REPO_URL, DOCUSAURUS_PLAYGROUND } from "@site/src/constants/v0.4.1";
...
```bash
git switch -c feature/<section_name> develop
```
...
```markdown title="<section_name>/<file_name>.mdx"
---
sidebar_position: 1
title: "<title>"
...
You can do it locally or by using <Link to={DOCUSAURUS_PLAYGROUND}>tools</Link> provided by Docusaurus.
...
Go to your docs fork and click "Pull request" to start one. The pull request should be from your branch to the `develop` branch of the original repository.
...
Once the pull request has been reviewed, the changes will be merged into the `develop` branch of the original repository.
...
```bash
git switch -c fix/<section_name> develop
```
