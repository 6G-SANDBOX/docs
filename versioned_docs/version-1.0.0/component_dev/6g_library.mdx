...
import { LIBRARY_GLOBAL_PAC_TN_DEPLOY, LIBRARY_GLOBAL_CAC_LOAD_VARIABLES, SITES_REPO_URL } from "@site/src/constants/v1.0.0";
...
The steps on how to structure a `component_playbook.yaml` are described in the [Playbook Development Guide](./playbook_deployment_guide.mdx).
...
Expanding step 3 and prior to the [Playbook Development Guide](./playbook_deployment_guide.mdx), it must be noted that there are multiple sources of variables available for a playbook.
