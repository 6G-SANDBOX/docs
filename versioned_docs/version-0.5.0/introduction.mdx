...
You can download or upgrade the appliances using the <Link to={TOOLKIT_INSTALLER_REPO_RELEASE}>toolkit installer</Link> or doing it manually. If you want to do it manually, remember to add the attributes to the images as explained above and update the <Link to={SITES_REPO_URL}>6G-Sandbox-Sites</Link> repository.
