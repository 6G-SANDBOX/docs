...
import { JENKINS_URL, JENKINS_LOGO, JENKINS_INSTALLATION, MARKETPLACE_APPLIANCE_JENKINS, SITES_REPO_URL, LIBRARY_GLOBAL_PAC_TN_DEPLOY, LIBRARY_GLOBAL_PAC_TN_DESTROY, ONE_DOCS_MANAGEMENT_OPERATIONS_CONTEXT_SECTION, ONE_DOCS_MANAGEMENT_OPERATIONS, ONE_APPS_WIKI_MINIO } from "@site/src/constants/v0.5.0";
...
- <Link to={MARKETPLACE_APPLIANCE_JENKINS}>jenkins</Link>
...
Contextualization parameters provided in the Virtual Machine template (<Link to={ONE_DOCS_MANAGEMENT_OPERATIONS_CONTEXT_SECTION}>CONTEXT section</Link>) control the initial VM configuration. Except for the common set of parameters supported by every appliance on the OpenNebula Marketplace, there are few specific to this particular jenkins appliance:
...
| `ANSIBLE_VAULT_PASSWORD`   | Password to encrypt and decrypt the <Link to={SITES_REPO_URL}>6G-Sandbox-Sites</Link> repository files for your site using Ansible Vault | \<undefined\> |
